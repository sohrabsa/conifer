  ...
  sampleCTMC <- function(init.state, duration) {
    time.passed <- 0
    current.state <- init.state
    pprint(current.state)
    # absorbing state
    if (Q[current.state, current.state] == 0) return(current.state)
        
    while (time.passed < duration) {
      # simulate wait time
      wait.time <- rexp(1, -Q[current.state, current.state])
      
      # choose a new state (shouldn't be the same one)
      current.state <- sample(states[states != current.state], 1, prob = Q[current.state, -c(current.state)])
  ...
  treeCTMC.edge.point <- function(tree, initial.state, edge.index, point) {
    ...
    for (i in edge.index:final.index) {
      result[tree$edge[i,2]] <- sampleCTMC(result[[tree$edge[i,1]]], tree$edge.length[i])
    }
  ...
  treeCTMC.edge.point.exclude <- function(tree, initial.state, edge.index, point) {
    ...
    result[tree$edge[edge.index, 2]] <- sampleCTMC(result[[tree$edge[i,1]]], point)
  ...
  driver <- function(tree, nSites, initialDistribution, rateMatrix, states, state.labels, mu) {
    ...
    for (i in seq(nSites)) {
      ...
      temp1 <- NULL
      if (runAbove & !(edgePoint[1] == 1 & edgePoint[2] == 0)) {
  ...
  example <- function() {
    ...
    rateMatrix <- deleteRateMatrix(1, size = length(states))
    print(rateMatrix)
